...
def softmax(x):
    ...
    ### YOUR CODE HERE
    max_element = tf.reduce_max(x, axis=1, keepdims=True)
    exp_x = tf.exp(x - max_element)
    out = exp_x / tf.reduce_sum(exp_x, axis=1, keepdims=True)
    ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    cross_entropy = -tf.reduce_sum(tf.to_float(y) * tf.log(yhat), axis=1)
    out = tf.reduce_sum(cross_entropy)
    ### END YOUR CODE
