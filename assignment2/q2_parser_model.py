...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        word_embedding = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(word_embedding, self.input_placeholder)
        embeddings = tf.reshape(embeddings, [tf.shape(self.input_placeholder)[0], -1])
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable(xavier_init(shape=(self.config.n_features * self.config.embed_size, self.config.hidden_size)))
        # W = xavier_weight_init(shape=(self.config.n_features, self.config.hidden_size))
        b1 = tf.Variable(tf.zeros(shape=(self.config.hidden_size,)))
        U = tf.Variable(xavier_init(shape=(self.config.hidden_size, self.config.n_classes)))
